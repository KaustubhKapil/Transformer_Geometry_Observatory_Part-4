...
from .topology import betti_curves, compute_diagrams, diagram_statistics
...
class Trainer:
    def __init__(self, cfg: Config, model, train_loader, val_loader, analysis_loader, trajectory_loader, device):
        self.cfg = cfg
        self.model = model
    ...
    def analyze_snapshot(self, epoch: int):
        selected_series, selected_tokens = self._capture_single_series(self.trajectory_loader)
        ...
        topology_stats = summarize_topology(diagrams_by_layer, np.linspace(0, 1.0, int(self.cfg.analysis.topology_eps_grid)))
        ...
        self._save_topology_plots(epoch, diagrams_by_layer, topology_stats)
    ...
    def _save_topology_plots(self, epoch: int, diagrams_by_layer: Sequence[Sequence[np.ndarray]], topology_stats: dict):
        ...
        for idx in selected_indices:
            ...
            eps_grid = np.linspace(0.0, 1.0, int(self.cfg.analysis.topology_eps_grid))
