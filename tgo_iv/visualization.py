...
def plot_trajectory_path(points: np.ndarray, path: str | Path, title: str = "", xlabel: str = "PC1", ylabel: str = "PC2"):
    ...
    if pts.ndim != 2 or pts.shape[0] == 0:
        ...
        return
    if pts.shape[1] < 2:
        padded = np.zeros((pts.shape[0], 2), dtype=np.float64)
        padded[:, : pts.shape[1]] = pts
        pts = padded
    ax.plot(pts[:, 0], pts[:, 1], linewidth=2, marker="o", markersize=3)
    ax.scatter([pts[0, 0]], [pts[0, 1]], s=50, label="start")
    ax.scatter([pts[-1, 0]], [pts[-1, 1]], s=50, label="end")
    ax.set_title(title)
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.grid(True, alpha=0.3)
...
def plot_persistence_diagram(diagrams: Sequence[np.ndarray], path: str | Path, title: str = "", max_value: Optional[float] = None):
    fig, ax = plt.subplots(figsize=(6.5, 6.0))
    ...
    for dim, dgm in enumerate(diagrams):
        ...
        if finite.size == 0:
            continue
        local_max = max(local_max, float(np.max(finite)))
        ax.scatter(finite[:, 0], finite[:, 1], s=16, alpha=0.8, label=f"H{dim}", color=colors[dim % len(colors)])
    axis_max = float(max_value) if max_value is not None else local_max
    if axis_max <= 0:
        axis_max = 1.0
...
def plot_barcode(diagrams: Sequence[np.ndarray], path: str | Path, title: str = "", max_value: Optional[float] = None):
    ...
    axis_max = float(max_value) if max_value is not None else max_death
    if axis_max <= 0:
        axis_max = 1.0
    axes[-1].set_xlabel("ε")
    axes[0].set_title(title)
    axes[-1].set_xlim(0, axis_max * 1.05)
    _save_fig(fig, path)
