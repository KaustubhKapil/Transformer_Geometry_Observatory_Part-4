...
def main():
    ...
    topo_path = output_dir / "topology_indices.npy"

    analysis_indices = np.arange(len(val_ds), dtype=np.int64)
    ...
    if topo_path.exists():
        topology_indices = np.load(topo_path)
    else:
        topology_indices = np.asarray([min(max(int(cfg.data.topology_image_index), 0), len(val_ds) - 1)], dtype=np.int64)
        np.save(topo_path, topology_indices)

    train_loader, val_loader, analysis_loader, trajectory_loader, topology_loader = build_loaders(
        train_ds,
        val_ds,
    ...
    trainer = Trainer(cfg, model, train_loader, val_loader, analysis_loader, trajectory_loader, topology_loader, device)
    if args.resume:
